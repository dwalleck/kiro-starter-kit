...
3. **Callers and consumers.** For any public or internal API change:
   - Use LSP `find references` and `find implementations` as the primary source.
...
7. **Build, test, and lint verification.** Run the project's standard check commands — a change that doesn't build or breaks tests has obvious findings you can discover cheaply. Consult steering files for project-specific commands; common defaults:
   ...
   Record outcomes and surface failures as findings at the appropriate severity.
...
1. **Verify the underlying type.** Many standard types are documented thread-safe (`ConcurrentDictionary`, `Arc<Mutex<T>>`, `ImmutableList`, `MemoryCache`, `Channel<T>`, etc.). Verify against documentation, not the type name.
